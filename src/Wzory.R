...
n<-length(pomiary)
Z<-qnorm(1-alpha/2)
# Teraz wyznaczamy lewy i prawy przedzia³ ufnoœci
...
pomiary<-c()
srednia<- mean(pomiary)
wartoœæ_oczekiwana<-sd(pomiary) # lub odchylenie standarodwe
# jeœli nie ma w poleceniu odch. stand. lub ani pomiary to na pewno bedzi wariancja
# S <- sqrt(wariancji)
poziom_ufnoœci<-
alpha<- 1-poziom_ufnoœci
n<-length(pomiary)
t<-qt(1-alpha/2, n-1)
# Teraz wyznaczamy lewy i prawy przedzia³ ufnoœci
L<-srednia - (wartoœæ_oczekiwana*t)/sqrt(n)
P<-srednia + (wartoœæ_oczekiwana*t)/sqrt(n)
Odp <- (L, P)


### X% przedzia³ ufnoœci
#Dane: potrzebujemy wariancji oraz X% poziom ufnoœci
pomiary <- c()
n <- length(pomiary)
poziom_ufnoœci <- 
alpha <- 1 - poziom_ufnoœci
