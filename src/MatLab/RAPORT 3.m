...
figure(3) % ZALE¯NO¦Æ CEN PRODUKTÓW
scatter(mleko, ser, 'r.')
xlabel(mleko_label)
ylabel(ser_label)
grid on
...
figure (6) % OBSZAR KRYTYCZNY DLA OBU PARAMETRÓW
hold on
grid on
box on
...
figure(7) % PROGNOZA PRZYSZ£YCH WARTO¦CI NA PODSTAWIE PROSTEJ REGRESJI BEZ UWZGLÊDNIENIA B£ÊDÓW STANDARDOWYCH
hold on
scatter(x, y, 'b.')
...
n=70;
figure(8); % % PROGNOZA PRZYSZ£YCH WARTO¦CI NA PODSTAWIE PROSTEJ REGRESJI BEZ UWZGLÊDNIENIA B£ÊDÓW STANDARDOWYCH Z PRZEDZIA£AMI UFNO¦CI
plot(X,Y, '.');
hold on;
...
figure(9) % PROGNOZA PRZYSZ£YCH WARTO¦CI NA PODSTAWIE PROSTEJ REGRESJI ZAK£ADAJ¡C BÊDY Z ROZKLADEM NORMALYM
hold on
scatter(x, y, 'b.')
scatter(x_next, y_next, 'o')
xlabel(mleko_label)
...
figure(11) % PORÓWNANIE ROZK£ADU RESIDUUM Z ROZK£ADEM NORMALNYM
grid on
box on
hold on
plot(res_x, res_y, 'b*')
...
figure(12) % PORÓWNANIE GÊSTO¦CI ROZK£ADU RESIDUUM Z GÊSTO¦CI¡ ROZK£ADU NORMALNEGO
grid on
box on
hold on
plot(res_x, res_y, 'b*')
...
title('Porownanie gêsto¶ci rozkladu residuum zgêsto¶ci± rozkladu normalnego')
...
figure(13) % PORÓWNANIE DYSTRYBUANTY EMPIRYCZNEJ ESTYMATORÓW B_0 Z DYSTRYBUANT¡ ROZK£ADU NORMALNEGO
grid on
box on
hold on
plot(b0_x, b0_y, 'b*')
...
figure(14) % PORÓWNANIE DYSTRYBUANTY EMPIRYCZNEJ ESTYMATORÓW B_1 Z DYSTRYBUANT¡ ROZK£ADU NORMALNEGO
grid on
box on
hold on
plot(b1_x, b1_y, 'b*')
